...
async function handleRequest(request, env, ctx) {
    ...
    if (url.pathname.startsWith('/images/')) {
      ...
      // مهم: لازم نحدد Content-Length صراحةً (حجم الصورة بالبايت). من غيره الاستجابة بتترسل بطريقة
      // "chunked" من غير ما فيسبوك يعرف حجم الصورة مقدمًا، وخطوة تحميل الصورة النهائية بتاعة فيسبوك وقت
      // النشر الفعلي (مختلفة عن مجرد المعاينة) بترفض الصور اللي من غير الهيدر ده أحيانًا.
      headers.set('Content-Length', String(object.size));
      return new Response(object.body, { headers });
    }

    // ===== بروكسي صور خارجية (لمعاينات المشاركة بس) =====
    // بعض المنتجات القديمة صورها مرفوعة على مواقع استضافة خارجية (زي phototourl.com وأشباهها)،
    // ومواقع زي دي غالبًا بتعمل حماية ضد الروابط الخارجية (hotlink protection) بترفض بيها أي طلب
    // من روبوتات المعاينة (فيسبوك/واتساب) حتى لو الصورة بتفتح عادي في متصفح عادي.
    // المسار ده بيخلي *سيرفرنا هو اللي يجيب الصورة* من المصدر الخارجي ويعرضها من على دومين موقعنا،
    // فروبوت فيسبوك بيطلبها من عندنا (موثوق) بدل ما يطلبها من المصدر الخارجي (اللي ممكن يرفضه).
    if (url.pathname === '/api/image-proxy') {
      const src = url.searchParams.get('url');
      if (!src) return new Response('رابط الصورة ناقص', { status: 400 });
      let srcUrl;
      try { srcUrl = new URL(src); } catch (e) { return new Response('رابط صورة غير صالح', { status: 400 }); }
      if (srcUrl.protocol !== 'https:' && srcUrl.protocol !== 'http:') {
        return new Response('رابط صورة غير صالح', { status: 400 });
      }
      try {
        const upstream = await fetch(srcUrl.toString(), {
          headers: {
            // بعض الاستضافات بترفض الطلبات اللي معاها User-Agent روبوتات (زي facebookexternalhit)،
            // فبنبعت الطلب من السيرفر بـ User-Agent متصفح عادي بدل ما نمرر هيدرز الزائر الأصلي.
            'User-Agent': 'Mozilla/5.0 (compatible; KARYMERA-ImageProxy/1.0)',
            'Accept': 'image/*',
          },
        });
        if (!upstream.ok || !upstream.body) {
          return new Response('تعذر تحميل الصورة من المصدر', { status: 502 });
        }
        const contentType = upstream.headers.get('Content-Type') || 'image/jpeg';
        // بنجيب حجم الصورة كامل في الذاكرة بدل ما نعمل تمرير مباشر (streaming) للبودي، عشان نقدر نحدد
        // Content-Length صراحةً - فيسبوك وقت النشر الفعلي (مش مجرد المعاينة) بيرفض أحيانًا الصور اللي
        // بترسل من غير الهيدر ده (استجابة "chunked" من غير حجم معروف مقدمًا).
        const imageBytes = await upstream.arrayBuffer();
        const headers = new Headers();
        headers.set('Content-Type', contentType);
        headers.set('Content-Length', String(imageBytes.byteLength));
        // كاش لمدة يوم بس (مش زي صور R2 اللي عندنا) لأن المصدر الخارجي ممكن يتغيّر من غير ما نتحكم فيه
    ...
    if (url.pathname === '/' || url.pathname === '/index.html') {
      ...
      // لو مفيش منتج محدد، ومفيش صورة مشاركة عامة، ومفيش أي منتجات أصلاً نحقنها، نسيب الصفحة زي ما هي
      if (!product && !siteShareImage && productsList.length === 0) return htmlResponse;

      const shareUrl = url.toString();
      // صورة المشاركة (لواتساب/فيسبوك) بتاعة أي منتج: دايمًا صورته الرئيسية هو بس (نفس الصورة المعروضة في كارت المنتج
      // في الموقع العام)، مهما كان له عنصر مرتبط في الشريط المتحرك وأيًا كانت صورة/سعر/تفاصيل ذلك العنصر.
      // بالشكل ده منتجات الشريط المتحرك مستقلة تمامًا عن منتجات الموقع العامة ومفيش أي تأثير متبادل بينهم.
      let imageToUse = product ? product.mainImg : siteShareImage;
      // لو الصورة مش مستضافة على دومين موقعنا (يعني مرفوعة برابط خارجي زي phototourl.com وأشباهها)،
      // نمررها من خلال بروكسي الصور بتاعنا عشان نضمن إن فيسبوك/واتساب يقدروا يوصلولها دايمًا.
      if (imageToUse) {
        try {
          const imgHost = new URL(imageToUse).hostname;
          if (imgHost !== url.hostname) {
            imageToUse = `${url.origin}/api/image-proxy?url=${encodeURIComponent(imageToUse)}`;
          }
        } catch (e) { /* رابط الصورة مش URL صالح، سيبها زي ما هي */ }
      }
      ...
      class HeadStructuredData {
        element(el) {
          ...
          const jsonLd = {
            ...
            '@type': 'Product',
            name: product.name,
            image: [product.mainImg].filter(Boolean),
