...
async function handleRequest(request, env, ctx) {
    ...
    if (url.pathname === '/' || url.pathname === '/index.html') {
      ...
      // ترتيب الأولوية لصورة المشاركة (لواتساب/فيسبوك):
      // 1) صورة مشاركة مخصصة حطها الأدمن يدويًا (مقاس عريض 1200x630) - لها الأولوية دايمًا ومفيش حاجة تكتب فوقها
      // 2) لو مفيش صورة مخصصة، وفيه عنصر في الشريط المتحرك مرتبط بنفس المنتج وليه صورة، تتستخدم هي كبديل أفضل من الصورة العادية
      // 3) لو مفيش أي حاجة من الاتنين، يرجع يستخدم صورة المنتج الرئيسية زي ما كان بيحصل قبل كده
      let carouselImageForProduct = null;
      if (product && Array.isArray(store?.carouselItems)) {
        const linkedItem = store.carouselItems.find((it) => it.sourceProductId === product.id && it.mainImg);
        if (linkedItem) carouselImageForProduct = linkedItem.mainImg;
      }
      const imageToUse = product ? (product.shareImg || carouselImageForProduct || product.mainImg) : siteShareImage;
      ...
      class MetaRewriter {
        element(el) {
          ...
          } else if (prop === 'og:image:width' || prop === 'og:image:height') {
            // الأبعاد دي (1200x630) بتاعة صورة الموقع الافتراضية بس، ومش هي نفسها أبعاد صورة المنتج الحقيقية.
            // سايبينها زي ما هي كانت بتخلي واتساب/فيسبوك يحسبوا القص غلط ويقصوا جزء من صورة المنتج.
            // بشيلها هنا عشان المنصة تكتشف أبعاد الصورة الصح من الصورة نفسها وتعرضها كاملة بدون قص خاطئ.
            el.remove();
          } else if (prop === 'og:url') {
