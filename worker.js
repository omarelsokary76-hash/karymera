...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      // مؤقت للتشخيص فقط: بيوري تفاصيل الخطأ الحقيقي في المتصفح بدل صفحة "1101" الفاضية
      return new Response(
...
async function handleRequest(request, env, ctx) {
    ...
    // ===== تسجيل دخول الأدمن: التحقق بقى في السيرفر بدل المتصفح، عشان الـ hash متبقاش ظاهرة في كود الصفحة العام =====
    if (url.pathname === '/api/login' && request.method === 'POST') {
      if (!env.ADMIN_HASH) return json({ error: 'ADMIN_HASH غير مضبوط' }, 500);
      let body;
    ...
    if (url.pathname === '/' || url.pathname === '/index.html') {
      ...
      // لو الأدمن حدد صورة مشاركة مخصصة (مقاس عريض 1200x630) بتستخدم هي عشان تظهر كاملة على واتساب/فيسبوك،
      // ولو مش متحددة بيرجع يستخدم الصورة الرئيسية العادية زي ما كان بيحصل قبل كده
      const imageToUse = product ? (product.shareImg || product.mainImg) : siteShareImage;
